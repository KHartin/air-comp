##Code that draws individual .csv files into one file

#Load the required packages - add the other packages for this analysis

pkgs <- c("knitr", "XLConnect", "doBy", "beeswarm")
for (pkg in pkgs) {
...
#Configure knitr options - From Brians Repo - Necessary?

opts_chunk$set(tidy=FALSE, cache=FALSE)

#Create the data folder - probably don't need this if folder already exists
...
#Creating file of Arithmetic Mean of values for each site
...
is.home <- subset(am.site, location == "Home")
is.ambient <- subset(am.site, location == "Ambient")
...
plot(am.site$conc~am.site$sitenum,log="y",ylab="Concentration ug/m^3", xlab="Site Number",
...
boxplot(am.site$conc~am.site$village, pch=19, ylab="Concentration ug/m^3", main="Conc by Village", 
        log="y", ylim=c(10,1000))
beeswarm(am.site$conc~am.site$village, pch=19, pwcol=ifelse(am.site$location=="Home",1,8), log="TRUE", 
